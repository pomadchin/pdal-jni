...
class PointView extends Native {
  ...
  def getX(packedPoint: Array[Byte]) = get(packedPoint)(DimType.X).getDouble
  def getY(packedPoint: Array[Byte]) = get(packedPoint)(DimType.Y).getDouble
  def getZ(packedPoint: Array[Byte]) = get(packedPoint)(DimType.Z).getDouble
